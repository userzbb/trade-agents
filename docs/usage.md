...
| 环境自检 / 修环境变量 / 为什么 hummingbot 连不上 | skill → `envcheck.mjs`（会话首个交易请求自动跑一次） | 环境变量状态表（当前进程 vs Windows 用户环境）；缺的必需变量 → setx 方案待你 CONFIRM |
...
node envcheck.mjs                    # 环境自检（当前进程 vs Windows 用户环境；缺的必需变量给 setx 建议）
